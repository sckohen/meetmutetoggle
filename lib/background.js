...
function getMeetingId(url) {
  let match = url.match(/https?:\/\/meet\.google\.com\/(?:_meet\/)?(\w+-\w+-\w+)(?:\?.*$)?/)
  meetId = match ? match[1] : 'none'
  return meetId
}
...
function messageHandler(request, sender, sendResponse) {
  console.log('we got a msg', request)
  if (request.action === 'leaving') {
    checkRemovedTab(sender.tab.id, null)
    return
  } else if (request.action === 'update') {  
    if (sender.tab) {
      const meetId = getMeetingId(sender.url)
      const meeting = {
        meetId: meetId,
        tabId: sender.tab.id,
        muted: request.muted,
...
function hotkeyHandler(cmd, tab) {
  console.log('hotkey command:', cmd)
  switch (cmd) {
    case 'toggle-mute':
      console.log('hotkey toggle-mute')
      toggleMute()
      break
    case 'toggle-cam':
      console.log('hotkey toggle-cam')
      toggleCam()
      break
    default:
